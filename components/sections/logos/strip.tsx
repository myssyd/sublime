      ...
      <div className="max-w-6xl mx-auto">
        <p
          className={applyElementOverrides(
            "text-center text-sm uppercase tracking-wider opacity-60 mb-8",
            "headline",
            styleOverrides
          )}
          ...
          {(content.logos || []).map((logo, index) => {
            const LogoImage = (
              <img
                ...
                alt={logo.alt}
                className={applyElementOverrides(
                  "h-8 md:h-10 w-auto opacity-60 hover:opacity-100 transition-opacity grayscale hover:grayscale-0",
                  `logos[${index}]`,
                  styleOverrides
                )}
              />
